...
public class ChatReportEvidence extends BaseEntity {
    ...
    @JoinColumn(name = "report_id", nullable = false,
            foreignKey = @ForeignKey(name = "fk_chat_report_evidence_report"))
    private Report report;

    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(name = "chat_room_id", nullable = false,
            foreignKey = @ForeignKey(name = "fk_chat_report_evidence_room"))
    ...
    private ChatReportEvidence(Report report, ChatRoom chatRoom, String firebaseMessageId,
                               Long senderUserId, String contentSnapshot, LocalDateTime sentAt) {
        this.report = report;
