...
@RestController
@RequestMapping("/v1/bal-game/{gameUuid}/comments")
...
public class BalCommentController {
    ...
    // 댓글/대댓글 작성 — gameUuid 기반 (parentId 가 있으면 대댓글)
    ...
    public ResponseEntity<RspTemplate<CommentRspDto>> createComment(
        ...
        CommentRspDto data = balGameCommentService.createComment(gameUuid, userId, req);
    ...
    @GetMapping
    public ResponseEntity<RspTemplate<List<CommentRspDto>>> getComments(@PathVariable String gameUuid) {
        Long userId = securityUtil.getCurrentUserId();
        List<CommentRspDto> data = balGameCommentService.getComments(gameUuid, userId);
    ...
    public ResponseEntity<RspTemplate<List<CommentRspDto>>> getRecentComments(
        ...
        List<CommentRspDto> data = balGameCommentService.getRecentComments(gameUuid, viewerId, size);
        return ResponseEntity.ok(new RspTemplate<>(HttpStatus.OK, "최신 댓글 조회 성공", data));
    }

    // 본인 댓글 삭제 — commentUuid 기반
    @Operation(summary = "본인 댓글 삭제")
    @DeleteMapping("/{commentUuid}")
    public ResponseEntity<RspTemplate<Void>> deleteComment(@PathVariable String gameUuid,
                                                           @PathVariable String commentUuid) {
        Long userId = securityUtil.getCurrentUserId();
        balGameCommentService.deleteComment(gameUuid, commentUuid, userId);
        return ResponseEntity.ok(new RspTemplate<>(HttpStatus.OK, "댓글 삭제 성공"));
    }

    // 댓글 좋아요
    @Operation(summary = "댓글 좋아요")
    @PostMapping("/{commentUuid}/likes")
    public ResponseEntity<RspTemplate<Void>> createLike(@PathVariable String gameUuid,
                                                        @PathVariable String commentUuid) {
        Long userId = securityUtil.getCurrentUserId();
        balCommentLikeService.createLike(commentUuid, userId);
        return ResponseEntity.status(HttpStatus.CREATED)
    ...
    @DeleteMapping("/{commentUuid}/likes")
    public ResponseEntity<RspTemplate<Void>> deleteLike(@PathVariable String gameUuid,
                                                        @PathVariable String commentUuid) {
        Long userId = securityUtil.getCurrentUserId();
        balCommentLikeService.deleteLike(commentUuid, userId);
