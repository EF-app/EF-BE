...
public class PostLikeController {
    ...
    @PostMapping
    public RspTemplate<PostLikeRspDto> createLike(@PathVariable Long postId) {
        Long userId = securityUtil.getCurrentUserId();
        PostLikeRspDto data = postLikeService.createLike(postId, userId);
        return new RspTemplate<>(HttpStatus.CREATED, "좋아요 성공", data);
    }

    @Operation(summary = "포스트잇 좋아요 취소", description = "본인이 누른 좋아요를 취소. 갱신된 likeCount/likedByMe 반환.")
    @DeleteMapping
    public RspTemplate<PostLikeRspDto> deleteLike(@PathVariable Long postId) {
        Long userId = securityUtil.getCurrentUserId();
        PostLikeRspDto data = postLikeService.deleteLike(postId, userId);
        return new RspTemplate<>(HttpStatus.OK, "좋아요 취소 성공", data);
    }
}
