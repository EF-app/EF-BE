...
public class PostItController {
    ...
    @PostMapping
    public RspTemplate<PostItRspDto> createPostIt(@Valid @RequestBody PostCreateReqDto req) {
        Long userId = securityUtil.getCurrentUserId();
        PostItRspDto data = postItService.createPostIt(userId, req);
        return new RspTemplate<>(HttpStatus.CREATED, "포스트잇 작성 성공", data);
    }

    @Operation(summary = "포스트잇 피드 조회", description = "커서 기반, 카테고리 옵션")
    @GetMapping
    public RspTemplate<CursorPageResponse<PostItRspDto>> getPostIts(
        ...
        return new RspTemplate<>(HttpStatus.OK, "포스트잇 목록 조회 성공", data);
    }

    @Operation(summary = "포스트잇 단건 상세", description = "포스트잇 단건을 id 로 상세 조회합니다. 비로그인도 호출 가능.")
    @GetMapping("/{postId}")
    public RspTemplate<PostItRspDto> getOnePostIt(@PathVariable Long postId) {
        Long viewerId = securityUtil.getCurrentUserIdOrNull();
        PostItRspDto data = postItService.getOnePostIt(postId, viewerId);
        return new RspTemplate<>(HttpStatus.OK, "포스트잇 상세 조회 성공", data);
    }

    @Operation(summary = "포스트잇 삭제", description = "본인 포스트잇을 id 로 Soft delete. 연결된 채팅방은 활성 유지.")
    @DeleteMapping("/{postId}")
    public RspTemplate<Void> deletePostIt(@PathVariable Long postId) {
        Long userId = securityUtil.getCurrentUserId();
        postItService.deletePostIt(postId, userId);
        return new RspTemplate<>(HttpStatus.OK, "포스트잇 삭제 성공");
    }

    @Operation(summary = "포스트잇 상단 고정", description = "POST_PIN 아이템을 소비해 본인 포스트잇(id)을 일정 시간 동안 상단 고정.")
    @PostMapping("/{postId}/pin")
    public RspTemplate<PostItRspDto> activatePin(@PathVariable Long postId) {
        Long userId = securityUtil.getCurrentUserId();
        PostItRspDto data = postItService.activatePin(postId, userId);
        return new RspTemplate<>(HttpStatus.OK, "상단 고정 성공", data);
    }
}
